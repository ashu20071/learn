...
public class OpenLock {
    public int openLock(String[] deadends, String target) {
        ...
        List<String> dE = Arrays.asList(deadends.clone());
        int steps = 0;
        String curr = "0000";
        if (!dE.contains(curr)){
            queue.add(curr);
            ...
            while (!queue.isEmpty()){
                ...
                for (int i=0; i<size; i++){
                    ...
                    if (queue.peek().equals(target)){
                        ...
                        addNeighbours(queue, dE, visited);
                        queue.poll();
                    }
    ...
    private void addNeighbours(Queue<String> queue, List<String> dE, List<String> visited) {

        assert queue.peek() != null;
        char[] chs = queue.peek().toCharArray();
        String s = "";
        ...
        for (int i=0; i<chs.length; i++){
            ...
            if (chs[i] == '9'){
                ...
                System.arraycopy(chs, 0, chars, 0, chs.length);
                int n = Integer.parseInt(String.valueOf(chars[i]));
                chars[i] = (char) (n - 1 + '0');
                for (char aChar : chars) {
                    s = s.concat(String.valueOf(aChar));
                }
                System.out.println("S="+s);
                if (!dE.contains(s) && !queue.contains(s) && !visited.contains(s)){
                    queue.add(s);
                    visited.add(s);
                }
                s="";
                ...
                char[] chars2 = new char[chs.length];
                System.arraycopy(chs, 0, chars2, 0, chs.length);
                int n = Integer.parseInt(String.valueOf(chars2[i]));
                chars2[i] = (char) (n + 1 + '0');
                ...
                char[] chars1 = new  char[chs.length];
                System.arraycopy(chs, 0, chars1, 0, chs.length);
                int n = Integer.parseInt(String.valueOf(chars1[i]));
                chars1[i] = (char) (n + 1 +'0');
